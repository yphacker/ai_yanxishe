...
def predict():
    ...
    submission_df = pd.DataFrame({"id": range(len(pred_list)), "label": pred_list})
...
def main(op):
    if op == 'train':
        ...
        # train_df['label'] = train_df['label'].apply(lambda x: config.label2id[x] if x in config.label2id else -1)
