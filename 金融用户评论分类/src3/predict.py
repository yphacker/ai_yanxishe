...
def file2submission_plus(files):
    preds_dict = dict()
    file_list = files.split(',')
    for i, filename in enumerate(file_list):
        df = pd.read_csv(filename, names=['id', 'label'])
        preds_dict[i] = df['label'].tolist()
...
if __name__ == '__main__':
    ...
    if args.pred_type == 'model':
        ...
        file2submission()
    elif args.pred_type == 'files':
        file2submission_plus(args.files)
