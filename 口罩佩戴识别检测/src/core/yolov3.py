...
class darknet53(object):
    ...
    def _darknet53_block(self, inputs, filters):
        ...
        :param inputs: 输入数据
        :param filters: 卷积核个数
        :return: 两次卷积后数据变为尺寸变为原来的1/2，且是和输入数据有关的
        """
        shortcut = inputs
        inputs = common._conv2d_fixed_padding(inputs, filters * 1, 1) #same
        inputs = common._conv2d_fixed_padding(inputs, filters * 2, 3)#下采样
    ...
    def forward(self, inputs):
        ...
        :return: 经过53层之后的输出
        """
        inputs = common._conv2d_fixed_padding(inputs, 32,  3, strides=1) #same
...
class yolov3(object):
    ...
    def __init__(self, num_classes, anchors,
        ...
        #TODO：YOLOv3类实现
        YOLO v3算法参数初始化
        :param num_classes: 多少类
        :param anchors: anchor
        :param batch_norm_decay: 批规范化衰减系数
        :param leaky_relu: leaky_relu系数
        """
    ...
    def _upsample(inputs, out_shape):
        ...
        # TODO：上采样层
        :param inputs: 输入数据
        :param out_shape: 输出数据的形状
        :return: 上采样后数据（通过双线性插值算法对输入数据进行插值）
        """
        new_height, new_width = out_shape[1], out_shape[2]
        inputs = tf.image.resize_nearest_neighbor(inputs, (new_height, new_width))
        inputs = tf.identity(inputs, name='upsampled') #Return a tensor with the same shape and contents as input.
    ...
    def forward(self, inputs, is_training=False, reuse=False):
        """
        #TODO:yolo v3模型构建，前向传播，输出三种尺寸的特征图
        #构建YOLO v3模型
        ...
        self.img_size = tf.shape(inputs)[1:3] #h,w
        # set batch norm params 批量规范化参数
        batch_norm_params = {
    ...
    def _reshape(self, x_y_offset, boxes, confs, probs):
        """
        #TODO：改变数据的形状
        :param x_y_offset: 中心偏移
        :param boxes: #矩形框的左上点和右下点坐标
        :param confs: 置信度
        :param probs: 概率值
    ...
    def compute_loss(self, pred_feature_map, y_true, ignore_thresh=0.5, max_box_per_image=8):
        """
        #TODO：计算loss最后的输出 [total_loss, loss_xy, loss_wh, loss_conf, loss_class]
        Note: compute the loss
        Arguments: y_pred, list -> [feature_map_1, feature_map_2, feature_map_3]
                                        the shape of [None, 13, 13, 3*85]. etc
        :param pred_feature_map: 特征图
        :param y_true: 真实标签
        :param ignore_thresh: #忽略的IOU值
        :param max_box_per_image: #每张图最大box
    ...
    def loss_layer(self, feature_map_i, y_true, anchors):
        ...
        # shape: [N, 13, 13, 3, 1]
        xy_loss = tf.reduce_sum(tf.square(true_xy    - pred_xy) * object_mask * box_loss_scale) / N # x,y loss
        wh_loss = tf.reduce_sum(tf.square(true_tw_th - pred_tw_th) * object_mask * box_loss_scale) / N #w,h loss
        ...
        conf_loss = tf.reduce_sum(conf_loss_pos + conf_loss_neg) / N #confs_loss =pos loss +neg loss

        # shape: [N, 13, 13, 3, 1]
        class_loss = object_mask * tf.nn.sigmoid_cross_entropy_with_logits(labels=y_true[..., 5:], logits=pred_prob_logits)
        class_loss = tf.reduce_sum(class_loss) / N   #class loss
