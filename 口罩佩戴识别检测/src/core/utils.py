...
#   Created date: 2018-11-22 12:02:52 ;2019-6-21 10:29:01
#   Description :
#
#================================================================
...
def gpu_nms(boxes, scores, num_classes, max_boxes=50, score_thresh=0.3, iou_thresh=0.5):
    ...
    :param max_boxes: -- integer, maximum number of predicted boxes you'd like, default is 50
    ...
    max_boxes = tf.constant(max_boxes, dtype='int32') #预测框的数量50

    # since we do nms for single image, then reshape it
    boxes = tf.reshape(boxes, [-1,4]) # '-1' means we don't konw the exact number of boxes
    # confs = tf.reshape(confs, [-1,1])
    score = tf.reshape(scores, [-1,num_classes]) #'-1' means we don't konw the exact number of boxes
...
def py_nms(boxes, scores, max_boxes=50, iou_thresh=0.5):
    ...
    :param boxes: shape of [-1, 4], the value of '-1' means that dont know the
                        exact number of boxes
    :param scores: shape of [-1,]
    :param max_boxes: epresenting the maximum of boxes to be selected by non_max_suppression
    :param iou_thresh:  representing iou_threshold for deciding to keep boxes
    :return: keep boxes default is 50
    """
    assert boxes.shape[1] == 4 and len(scores.shape) == 1 # let boxes's just have x1,y1,x2,y2 and scores's shape just equal one

    # get boxes's x1,y1,x2,y2
    x1 = boxes[:, 0]
    y1 = boxes[:, 1]
    x2 = boxes[:, 2]
    y2 = boxes[:, 3]
...
def draw_boxes(image, boxes, scores, labels, classes, detection_size,
    ...
    for i in range(len(labels)): # for each bounding box, do:
        ...
        # # draw bbox
        #TODO 实现切割车牌区域 [tuple(text_origin), tuple(text_origin+text_size)
        #crop = image.crop(bbox)
        # crop = image.crop((bbox[0]-2,bbox[1]-2,bbox[2]+2,bbox[3]+2))
        # crop.save("./car_region/plate.jpg")
