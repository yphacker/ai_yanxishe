...
#
#================================================================
#todo: python convert_tfrecord.py  --dataset_txt ../../keras_yolo3/master/data/plate_train_data.txt
import sys
import argparse
import numpy as np
...
def main(argv):
    ...
    with open(flags.dataset_txt,'r') as f:
        for line in f.readlines():
            ...
            print(boxes_num)
            boxes = np.zeros([boxes_num, 5],np.float32)
            for i in range(boxes_num):
                boxes[i] = example[1+i*5:6+i*5]
            dataset[str(image_path)] = boxes
