...
def load_data():
    ...
    movies = item_df[['movie_id', 'movie_title', 'unknowngenres', 'Action', 'Adventure',
    ...
    title_map = {val: (pattern.match(val).group(1) if pattern.match(val) else '') for ii, val in
                 enumerate(set(movies['movie_title']))}
    ...
    # 读取评分数据集
    train_df = pd.read_table('../data/train.csv', sep=',', names=['userId', 'movie_id', 'ratings', 'timestamps'])
    ratings = train_df.filter(regex='userId|movie_id|ratings')

    # 合并三个表
    data = pd.merge(pd.merge(ratings, user_df), item_df)
    ...
    genres2int = {}
    return title_count, title_set, genres2int, features, targets_values, ratings, user_df, item_df, data, movies_orig, users_orig
