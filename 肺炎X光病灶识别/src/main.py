...
def main(op):
    if op == 'train':
        train_df = pd.read_csv('../data/train.csv', header=None, names=['filename', 'label'])
        print(train_df['label'].value_counts())
        train_df = train_df[train_df['label'] < 3]
        train_df['filename'] = train_df['filename'].apply(lambda x: '../data/train/{0}'.format(x) + '.jpg')
        if mode == 1:
            train_data, val_data = train_test_split(train_df, shuffle=True, test_size=0.1, random_state=0)
